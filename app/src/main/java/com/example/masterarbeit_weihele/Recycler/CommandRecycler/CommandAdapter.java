...
public class CommandAdapter extends RecyclerView.Adapter<CommandViewHolder> {
    ...
    private final List<Command_Item> items;
    ...
    public void onBindViewHolder(@NonNull CommandViewHolder holder, int position) {
        ...
        if(command_item.getCompleted()){
            setItemCompleted(command_item, holder.commandView);
        } else if(command_item.getIsFocused()){
    ...
    public void moveItemToFront(int sourcePosition) {
        Command_Item item = items.get(sourcePosition);
        boolean isFirstItemFocused = items.get(0).getIsFocused();
        items.remove(sourcePosition);
    ...
    public void saveItems(){
        SharedPreferences sharedPreferences = context.getSharedPreferences("Commands", Context.MODE_PRIVATE);
        SharedPreferences.Editor editor = sharedPreferences.edit();
        for(int i = 0; i<items.size(); i++){
            editor.putBoolean("Item_isFocused_" + i, items.get(i).getIsFocused());
            editor.putBoolean("Item_isCompleted_" + i, items.get(i).getCompleted());
